...
<body>   
    <div class="container">      
            ...
            <h3 class="text-center">REPORTE HISTORIAL DE DÍA </h3>     
        ...
        <table class="table table-condensed table-bordered ">
            <thead>
                ...
                <tr>
                    <th>COTIZACIONES</th>   
                    @foreach ($num_fact as $c)
                    <td>{{ $c->cantidad }}</td>     
                    @endforeach     
                </tr>
                <tr>
                    <th>PRE-ORDEN</th>   
                    @foreach ($num_pre as $c)
                    <td>{{ $c->cantidad }}</td>     
                    @endforeach               
                </tr>
                ...
                <tr>
                    <th>COMPRA</th>   
                    @foreach ($num_com as $c)
                    <td>{{ $c->cantidad }}</td>     
                    @endforeach               
                </tr>
                <tr>
                     <th>AGENDADOS</th>
                     @foreach ($num_agen as $c)
                     <td>{{ $c->cantidad }}</td>     
                     @endforeach     
                </tr>
                <tr>
                    <th>ENTREGADOS</th>
                    @foreach ($num_ent as $c)
                    <td>{{ $c->cantidad }}</td>     
                    @endforeach     
               </tr>
               <tr>
