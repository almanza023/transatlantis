...
class DepartamentSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($departaments as $departament) {
            $d = new Departament;
            $d->name_departament = $departament;
            $d->save();

            if ($d->name_departament == 'Amazonas') {
                foreach ($amazona as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Antioquia') {
                foreach ($antioquia as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Arauca') {
                foreach ($arauca as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Atlantico') {
                foreach ($atlantico as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Bolivar') {
                foreach ($bolivar as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Boyaca') {
                foreach ($boyaca as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Caldas') {
                foreach ($caldas as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Caqueta') {
                foreach ($caqueta as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Casanare') {
                foreach ($casanare as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Cauca') {
                foreach ($cauca as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Cesar') {
                foreach ($cesar as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Choco') {
                foreach ($choco as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Cordoba') {
                foreach ($cordoba as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Cundinamarca') {
                foreach ($cundinamarca as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Guainia') {
                foreach ($guainia as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Guaviare') {
                foreach ($guaviare as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Huila') {
                foreach ($huila as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'La Guajira') {
                foreach ($guajira as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Magdalena') {
                foreach ($magdalena as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Meta') {
                foreach ($meta as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Nariño') {
                foreach ($nariño as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Norte Santander') {
                foreach ($norte_santander as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Putumayo') {
                foreach ($putumayo as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Quindio') {
                foreach ($quindio as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Risaralda') {
                foreach ($risaralda as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'San Andres') {
                foreach ($san_andres as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Santander') {
                foreach ($santander as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Sucre') {
                foreach ($sucre as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Tolima') {
                foreach ($tolima as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Valle del Cauca') {
                foreach ($valleDelCauca as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Vaupes') {
                foreach ($vaupes as $municipio) {
                    Municipality::create(['name_municipality' => $municipio, 'id_departament' => $d->id_departament]);
                }
            }

            if ($d->name_departament == 'Vichada') {
